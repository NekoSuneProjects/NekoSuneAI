"""The node's live link to the backend, and its fallback behaviour.

The link exists so a long turn is not cut off by a proxy's read timeout. It is
opportunistic: every caller keeps its HTTP path, so these check just as hard
that failing over is clean as that connecting works.
"""
...
def make_client(backend, token="good-token", on_command=None):
    return WebSocketClient(
        server_url=backend.url, node_id="pi-1", token_provider=lambda: token,
...
def test_an_unpaired_node_does_not_burn_reconnect_attempts(backend):
    ...
    client = WebSocketClient(
        server_url=backend.url, node_id="pi-1", token_provider=lambda: "",
