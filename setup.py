"""
NekoSuneAI Setup — cross-platform installer (Windows + Linux).

    python setup.py              # full setup + launch GUI (or terminal if headless)
...
def preload_models() -> None:
    ...
    run([str(VENV_PYTHON), "-m", "nekosuneai.bootstrap"])
...
def full_setup() -> None:
    ...
    if resolve_install_profile() in {"voice", "full"}:
        ...
        step(7, total, "Skipping voice model preload (text-only install).")
    ...
    banner("NekoSuneAI Setup Complete")
    if not is_headless():
...
def run_update() -> None:
    ...
    if not VENV_PYTHON.exists():
        ...
        raise SystemExit(1)
    run([str(VENV_PYTHON), "-m", "nekosuneai.updater", "--apply"])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="NekoSuneAI — setup, launch, and update.",
    )
...
def main() -> None:
    ...
    if args.setup:
        ...
        # there's no window to show, so fall back to terminal mode.
        if not VENV_PYTHON.exists() or not SETUP_MARKER.exists():
            full_setup()
        if is_headless():
            launch_terminal()
