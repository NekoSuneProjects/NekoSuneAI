...
class ChromecastTarget(BaseMediaTarget):
    ...
    def _cast(self):
        ...
        chromecasts, browser = pychromecast.get_listed_chromecasts(friendly_names=[friendly], discovery_timeout=5)
        try:
            if not chromecasts:
                raise MediaTargetError(f"Chromecast '{friendly}' was not found on the LAN.")
            cast = chromecasts[0]
            cast.wait(timeout=10)
            return cast
        finally:
    ...
    def control(self, action: str, value: float | int | None = None) -> str:
        ...
        elif action == "seek": mc.seek(float(value or 0))
        elif action == "volume": cast.set_volume(_clamp_volume(int(value or 0)) / 100.0)
        else: raise MediaTargetError(f"Unsupported Chromecast action: {action}")
...
def _soap(url: str, service: str, action: str, fields: dict[str, Any]) -> requests.Response:
    body_fields = "".join(f"<{k}>{v}</{k}>" for k, v in fields.items())
    body = f'<?xml version="1.0"?><s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"><s:Body><u:{action} xmlns:u="urn:schemas-upnp-org:service:{service}:1">{body_fields}</u:{action}></s:Body></s:Envelope>'
    r = requests.post(url, data=body.encode("utf-8"), headers={"Content-Type": 'text/xml; charset="utf-8"', "SOAPACTION": f'"urn:schemas-upnp-org:service:{service}:1#{action}"'}, timeout=6)
    r.raise_for_status()
    return r
    ...
    def _services(self) -> DlnaServices:
        ...
        for location in candidates:
            try:
                ...
                continue
        raise MediaTargetError("No DLNA MediaRenderer found. Set DLNA_DEVICE_URL or DLNA_AVTRANSPORT_URL.")

    def play_url(self, url: str, *, title: str = "", content_type: str = "audio/mpeg") -> str:
        services = self._services()
        metadata = ""
        _soap(services.avtransport, "AVTransport", "SetAVTransportURI", {"InstanceID": 0, "CurrentURI": url, "CurrentURIMetaData": metadata})
        _soap(services.avtransport, "AVTransport", "Play", {"InstanceID": 0, "Speed": 1})
        return f"Playing {title or 'media'} on the DLNA renderer."

    def control(self, action: str, value: float | int | None = None) -> str:
        services = self._services()
        if action in {"play", "pause", "stop", "next", "previous"}:
            names = {"play": "Play", "pause": "Pause", "stop": "Stop", "next": "Next", "previous": "Previous"}
            fields = {"InstanceID": 0}
            if action == "play": fields["Speed"] = 1
            ...
            _soap(services.avtransport, "AVTransport", "Seek", {"InstanceID": 0, "Unit": "REL_TIME", "Target": target})
        elif action == "volume":
            if not services.rendering:
                raise MediaTargetError("This DLNA renderer did not publish RenderingControl.")
            _soap(services.rendering, "RenderingControl", "SetVolume", {"InstanceID": 0, "Channel": "Master", "DesiredVolume": _clamp_volume(int(value or 0))})
        else:
...
class AndroidTvTarget(BaseMediaTarget):
    ...
    KEYCODES = {"play": "126", "pause": "127", "stop": "86", "next": "87", "previous": "88", "toggle": "85"}
    ...
    def _adb(self, *args: str) -> str:
        ...
        if not host:
            raise MediaTargetError("Set ANDROID_TV_HOST to the TV's LAN address, optionally with :5555.")
        target = host if ":" in host else host + ":5555"
        subprocess.run([adb, "connect", target], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=8)
        cp = subprocess.run([adb, "-s", target, *args], capture_output=True, text=True, timeout=8)
        if cp.returncode != 0:
            raise MediaTargetError((cp.stderr or cp.stdout or "ADB command failed").strip())
    ...
    def control(self, action: str, value: float | int | None = None) -> str:
        if action in self.KEYCODES:
            ...
            self._adb("shell", "cmd", "media_session", "dispatch", "seekto", str(millis))
        elif action == "volume":
            percent = _clamp_volume(int(value or 0))
            # Android's media volume scale varies by device; use media CLI's percentage flag where supported.
            try:
                self._adb("shell", "media", "volume", "--stream", "3", "--set", str(round(percent * 15 / 100)))
            except MediaTargetError:
                key = "24" if percent >= 50 else "25"
...
class LgWebOsTarget(BaseMediaTarget):
    ...
    def _request(self, uri: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            register = {"type": "register", "id": "register_0", "payload": {"pairingType": "PROMPT", "manifest": {"manifestVersion": 1, "appVersion": "1.0", "signed": {"created": "20260905", "appId": "com.nekosuneai.remote", "vendorId": "NekoSuneProjects", "localizedAppNames": {"": "NekoSuneAI"}, "localizedVendorNames": {"": "NekoSuneProjects"}, "permissions": ["CONTROL_AUDIO", "CONTROL_INPUT_MEDIA_PLAYBACK", "CONTROL_POWER"]}, "permissions": ["CONTROL_AUDIO", "CONTROL_INPUT_MEDIA_PLAYBACK", "CONTROL_POWER"]}}}
            if key:
                register["payload"]["client-key"] = key
            ws.send(json.dumps(register))
            registered = json.loads(ws.recv())
            new_key = ((registered.get("payload") or {}).get("client-key") or "")
            if new_key and not key:
                print(f"[media] LG webOS paired. Set LG_WEBOS_CLIENT_KEY={new_key} to avoid re-pairing.")
            request_id = "neko_" + uuid.uuid4().hex[:10]
            ws.send(json.dumps({"type": "request", "id": request_id, "uri": uri, "payload": payload or {}}))
            for _ in range(4):
                result = json.loads(ws.recv())
                if result.get("id") == request_id:
                    if result.get("type") == "error":
                        raise MediaTargetError(str(result.get("error") or "webOS request failed"))
    ...
    def control(self, action: str, value: float | int | None = None) -> str:
        uris = {"play": "ssap://media.controls/play", "pause": "ssap://media.controls/pause", "stop": "ssap://media.controls/stop", "next": "ssap://media.controls/fastForward", "previous": "ssap://media.controls/rewind"}
        if action in uris:
            self._request(uris[action])
        elif action == "volume":
            self._request("ssap://audio/setVolume", {"volume": _clamp_volume(int(value or 0))})
        elif action == "seek":
            raise MediaTargetError("LG webOS does not expose a reliable absolute seek command through the generic SSAP media controls; use next/previous/fast-forward/rewind where the active app supports them.")
        else:
...
class SamsungTvTarget(BaseMediaTarget):
    ...
    KEYS = {"play": "KEY_PLAY", "pause": "KEY_PAUSE", "stop": "KEY_STOP", "next": "KEY_FF", "previous": "KEY_REWIND"}
    ...
    def _send_key(self, key: str, repeats: int = 1) -> None:
        ...
        name = base64.b64encode(_env("SAMSUNG_TV_NAME", "NekoSuneAI").encode()).decode()
        scheme = "wss" if _env("SAMSUNG_TV_SSL", "1").lower() not in {"0", "false", "no"} else "ws"
        port = 8002 if scheme == "wss" else 8001
        url = f"{scheme}://{host}:{port}/api/v2/channels/samsung.remote.control?name={name}" + (f"&token={token}" if token else "")
    ...
    def control(self, action: str, value: float | int | None = None) -> str:
        if action in self.KEYS:
            ...
            # Samsung's remote websocket exposes relative volume keys, not an absolute media-volume setter.
...
def get_media_target(name: str | None = None) -> BaseMediaTarget:
    ...
    if target == "chromecast": return ChromecastTarget()
    if target == "dlna": return DlnaTarget()
    if target == "android-tv": return AndroidTvTarget()
    if target == "lg-webos": return LgWebOsTarget()
    if target == "samsung-tv": return SamsungTvTarget()
    raise MediaTargetError(f"Unknown remote media target: {target}")


def control_remote_media(action: str, *, target: str | None = None, value: float | int | None = None) -> str:
    return get_media_target(target).control(action, value)


def play_remote_media(url: str, *, target: str | None = None, title: str = "", content_type: str = "audio/mpeg") -> str:
    return get_media_target(target).play_url(url, title=title, content_type=content_type)
