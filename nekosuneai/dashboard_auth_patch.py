...
def install_dashboard_auth_patch() -> None:
        ...
        def __init__(self, server_address, RequestHandlerClass, *args, **kwargs):
            ...
            class DashboardAuthHandler(base_handler):
                def _dashboard_authorized(self) -> bool:
                    # Once an admin password is configured, browser dashboard auth
                    # is cookie-based. Legacy ?token= remains only as a fallback
                    # when no password has been configured, preventing lockout on
                    # old local-only installs.
                    if _ADMIN_PASSWORD:
                ...
                def do_POST(self):
                    ...
                    if not _ADMIN_PASSWORD:
                        return _render_login(self, error="DASHBOARD_ADMIN_PASSWORD is not configured.")
                    ip = str(self.client_address[0] if self.client_address else "unknown")
                    if not _login_allowed(ip):
                        return _render_login(self, error="Too many failed sign-in attempts. Try again in a few minutes.")
                    try:
                        length = min(max(int(self.headers.get("Content-Length", "0")), 0), 8192)
                    except ValueError:
                        length = 0
                    ...
                    if not (hmac.compare_digest(username, _ADMIN_USER) and hmac.compare_digest(password, _ADMIN_PASSWORD)):
                        ...
                        return _render_login(self, error="Incorrect username or password.", next_path=next_path)
                    self.send_response(303)
                    self.send_header("Location", next_path if next_path.startswith("/") and not next_path.startswith("//") else "/")
                    self.send_header("Cache-Control", "no-store")
                    _set_session_cookie(self, _make_session())
                    self.end_headers()
                ...
                def do_GET(self):
                    ...
                    if path == "/logout":
                        self.send_response(303); self.send_header("Location", "/login"); self.send_header("Cache-Control", "no-store")
                        _set_session_cookie(self, "", clear=True); self.end_headers(); return
                    # Public read-only avatar display. Upload/management remains admin-only.
                    if path in {"/avatar", "/api/avatar/file"}:
                        return super().do_GET()
    ...
    # call_sync_patch imports webserver before this final wrapper is installed.
    # Rebind its local imported name so serve() actually creates our final server.
    try:
