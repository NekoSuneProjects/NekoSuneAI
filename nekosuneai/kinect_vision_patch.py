...
class KinectVisionService:
    ...
    face crop; a *relayed* description (via ``describe_callback`` -- Pi Proxy
    hands the JPEG to the Docker backend's `/api/nodes/media/vision` endpoint,
    it never calls a vision provider directly itself) can add visible
    posture/gesture context. Both outputs are explicitly tentative visual cues.

    Adapted from the original Docker-backend patch of the same name: that
    version read settings from the full backend's dashboard settings store
    and called `vision.describe_image` (a direct vision-provider API call)
    in-process. Pi Proxy has neither of those -- ``config`` is this node's own
    config dict, and ``describe_callback`` is Pi Proxy's own relay helper.
    """

    def __init__(self, config: dict[str, Any] | None = None, describe_callback: Callable[[bytes], str | None] | None = None) -> None:
        ...
        self._describe_callback = describe_callback

    def _settings(self) -> dict[str, Any]:
        s = self._config
        return {
            "enabled": _bool(s.get("kinect_vision_enabled"), False),
            "interval": max(1.0, float(s.get("kinect_vision_interval_seconds", 4.0) or 4.0)),
            "device": max(0, int(s.get("kinect_device_index", 0) or 0)),
            "describe": _bool(s.get("kinect_vision_describe"), True) and self._describe_callback is not None,
    ...
    def _analyse(self, jpeg: bytes, settings: dict[str, Any]) -> str:
        ...
        if settings["describe"] and self._describe_callback is not None:
            try:
                text = self._describe_callback(jpeg)
                if text:
                    parts.append("Kinect camera currently sees: " + text[:700])
            except Exception as exc:
                self._last_error = f"vision relay: {exc}"
    ...
    def capture_jpeg_once(self, device: int | None = None) -> bytes:
        """On-demand single-frame capture (e.g. a `camera.snapshot` request),
        independent of the background analysis loop's own interval/schedule."""
        return self._capture_jpeg(device if device is not None else self._settings()["device"])
...
# `install_kinect_vision_patch()` that monkey-patched `KinectVisionService`
# into `webgui.py`'s settings schema and `webserver.py`'s prompt summary --
# neither module exists on this branch. `pi_proxy_agent.py` constructs and
# drives `KinectVisionService` directly instead (see its `camera.status`/
