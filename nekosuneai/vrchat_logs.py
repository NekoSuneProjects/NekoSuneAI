...
calls (no rate limits, no credentials, no ToS-risky web API) and no GPU. This
only works run on the machine actually running the VRChat client, which is why
it lives on the Windows agent rather than the Pi/server backend.
"""
