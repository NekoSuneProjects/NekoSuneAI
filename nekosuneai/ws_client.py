...
Why this exists: a converse turn runs the whole backend pipeline -- web
search, the LLM, TTS -- and regularly outlives a reverse proxy's *read*
timeout, which answers this node 504 and throws away a reply the backend had
already finished. A proxy applies its much longer *idle* timeout to an
upgraded connection, and the keepalive below stops even that firing, so a turn
can take as long as it takes. It also lets the backend push a command the
moment it is queued instead of this node polling for it.
...
class WebSocketClient:
    ...
    def start(self) -> None:
        if self._thread is not None and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._run, daemon=True, name="pi-proxy-ws")
        self._thread.start()
    ...
    def _start_keepalive(self) -> threading.Event:
        ...
        threading.Thread(target=run, daemon=True, name="pi-proxy-ws-ping").start()
        return stop
    ...
    def _handle(self, message: dict[str, Any]) -> None:
        ...
        if kind == "command":
            ...
            threading.Thread(
                target=self._run_command, args=(command,), daemon=True, name="pi-proxy-ws-command",
            ).start()
            return
        if kind == "pong":
            return
