...
create_launchers() {
    ...
    # mode lives in .nova-run-mode so you can flip it later without re-installing.
    local run_script="$INSTALL_DIR/run-nova.sh"
    echo "${RUN_MODE:-cli}" > "$INSTALL_DIR/.nova-run-mode"
...
cd "\$HERE"
MODE="\$(cat "\$HERE/.nova-run-mode" 2>/dev/null || echo "${RUN_MODE:-cli}")"
...
RUNEOF
    ...
    if is_headless; then
        ok "No desktop detected — skipping menu shortcut. Start with: ./run-nova.sh"
        return
    fi
    if ! ask_yes_no "Create a desktop / menu shortcut for NekoSuneAI?"; then
        ok "No shortcut created. Start with: ./run-nova.sh"
        return
    fi
...
show_finish() {
    ...
    echo -e "      ${CYAN}./run-nova.sh${NC}          ${DIM}# starts in ${RUN_MODE:-cli} mode${NC}"
    echo -e "      ${DIM}…or pick ${WHITE}NekoSuneAI${NC}${DIM} from your application menu.${NC}"
    echo -e "      ${DIM}Switch modes later: edit ${WHITE}.nova-run-mode${NC}${DIM} (gui / cli).${NC}"
    echo ""
