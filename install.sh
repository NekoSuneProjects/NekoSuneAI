...
show_banner() {
    ...
    echo -e "         ${MAGENTA}o--${MAGENTA}|${NC}  ${WHITE}N e k o S u n e${NC}  ${MAGENTA}|${MAGENTA}--o${NC}"
...
install_system_deps() {
    ...
    if ! $SUDO $PKG_INSTALL "${pkgs[@]}"; then
        warn "Some system packages failed to install. NekoSuneAI may still work; install them manually if a feature is missing."
    else
        ok "System packages installed."
    fi
}

# Ask HOW NekoSuneAI should run — native GUI window or CLI/terminal chat. This
# single choice drives the dependency set and the launcher that gets created:
#
#   GUI → base + voice + desktop GUI   (profile: full)
#   CLI → base + voice                 (profile: voice)
#
# Both modes always include requirements.txt + requirements-voice.txt;
# GUI additionally installs requirements-gui.txt.
choose_run_mode() {
    local default_hint="On a headless Pi / server (no screen), choose CLI/terminal."
    is_headless && default_hint="No display detected — CLI/terminal mode is recommended."

    info "$default_hint"
    local choice
    choice=$(ask_choice "How do you want to run NekoSuneAI?" \
        "GUI  — native desktop window (needs a screen + desktop environment)" \
        "CLI  — terminal chat loop, works great headless (Pi / server)")

    case "$choice" in
        1) RUN_MODE="gui"; INSTALL_PROFILE="full";  NEED_VOICE=true; NEED_GUI=true ;;
        2) RUN_MODE="cli"; INSTALL_PROFILE="voice"; NEED_VOICE=true; NEED_GUI=false ;;
        *) RUN_MODE="cli"; INSTALL_PROFILE="voice"; NEED_VOICE=true; NEED_GUI=false ;;
    esac
    if [[ "$NEED_GUI" == "true" ]]; then
        ok "Run mode: GUI  (base + voice + desktop GUI)"
    else
        ok "Run mode: CLI  (base + voice)"
    fi
}
...
choose_llm_provider() {
    ...
    echo ""
    info "NekoSuneAI works with local and cloud LLMs."
    ...
    case "$choice" in
        ...
        4)
            ...
            info "Make sure LM Studio's local server is running before you start NekoSuneAI."
...
get_nekosuneai() {
    step "4/7" "Downloading NekoSuneAI..."

    if [[ -f "$INSTALL_DIR/setup.py" ]]; then
        ok "NekoSuneAI already exists at $INSTALL_DIR"
    ...
    if has_cmd git; then
        ...
        if has_cmd curl; then
            ...
            fail "Neither git, curl, nor wget found. Cannot download NekoSuneAI."
            exit 1
        fi
...
run_setup() {
    step "5/7" "Running NekoSuneAI setup..."
    ...
    (cd "$INSTALL_DIR" && NEKOSUNEAI_INSTALL_PROFILE="${INSTALL_PROFILE:-full}" "$python_cmd" setup.py --setup)
    ok "Setup complete."
}
...
# ── Step 7: Launcher + desktop entry (both GUI and CLI modes) ───────────────

create_launchers() {
    step "7/7" "Creating launcher (mode: ${RUN_MODE:-cli})..."

    # Convenience script that always starts NekoSuneAI in the chosen mode. The
    # mode lives in .nekosuneai-run-mode so you can flip it later without
    # re-installing.
    rm -f "$INSTALL_DIR/run-nova.sh" "$INSTALL_DIR/.nova-run-mode"
    local run_script="$INSTALL_DIR/run-nekosuneai.sh"
    echo "${RUN_MODE:-cli}" > "$INSTALL_DIR/.nekosuneai-run-mode"
    cat > "$run_script" <<RUNEOF
#!/usr/bin/env bash
# Auto-generated by install.sh — starts NekoSuneAI in its configured mode.
HERE="\$(cd "\$(dirname "\$(readlink -f "\$0" 2>/dev/null || echo "\$0")")" && pwd)"
cd "\$HERE"
MODE="\$(cat "\$HERE/.nekosuneai-run-mode" 2>/dev/null || echo "${RUN_MODE:-cli}")"
...
RUNEOF
    chmod +x "$run_script"
    ok "Launcher script: $run_script  (mode: ${RUN_MODE:-cli})"

    # A .desktop menu/desktop shortcut only makes sense with a graphical desktop.
    if is_headless; then
        ok "No desktop detected — skipping menu shortcut. Start with: ./run-nekosuneai.sh"
        return
    fi
    if ! ask_yes_no "Create a desktop / menu shortcut for NekoSuneAI?"; then
        ok "No shortcut created. Start with: ./run-nekosuneai.sh"
        return
    fi

    local desktop_dir="${XDG_DATA_HOME:-$HOME/.local/share}/applications"
    mkdir -p "$desktop_dir"
    local launcher_path="$desktop_dir/nekosuneai.desktop"
    local icon_path="$INSTALL_DIR/data/logo.png"
    [[ -f "$icon_path" ]] || icon_path="$INSTALL_DIR/nekosuneai/static/logo.png"

    local exec_line term_line comment
    if [[ "$RUN_MODE" == "gui" ]]; then
        exec_line="$run_script"
        term_line="false"
        comment="Launch NekoSuneAI desktop GUI"
    else
        # CLI mode: run the terminal chat loop in a terminal window.
        exec_line="$run_script"
        term_line="true"
        comment="Start NekoSuneAI in the terminal"
    fi
...
DESKTOP
    ...
    if [[ -d "$user_desktop" ]]; then
        cp "$launcher_path" "$user_desktop/NekoSuneAI.desktop"
        chmod +x "$user_desktop/NekoSuneAI.desktop"
    fi
    command -v update-desktop-database >/dev/null 2>&1 && \
        update-desktop-database "$desktop_dir" >/dev/null 2>&1 || true
    ok "Desktop shortcut created (${RUN_MODE} mode)."
}
...
show_finish() {
    ...
    echo -e "  ${GREEN}╔══════════════════════════════════════════╗${NC}"
    echo -e "  ${GREEN}║                                          ║${NC}"
    echo -e "  ${GREEN}║   ${WHITE}NekoSuneAI is ready to go!${NC}             ${GREEN}║${NC}"
    echo -e "  ${GREEN}║                                          ║${NC}"
    echo -e "  ${GREEN}╚══════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo ""
    echo -e "    ${DIM}Run mode:${NC}     ${WHITE}${RUN_MODE:-cli}${NC} ${DIM}(profile: ${INSTALL_PROFILE:-full})${NC}"
    echo ""
    echo -e "    ${DIM}To launch NekoSuneAI anytime:${NC}"
    echo ""
    echo -e "      ${CYAN}cd $INSTALL_DIR${NC}"
    echo -e "      ${CYAN}./run-nekosuneai.sh${NC}          ${DIM}# starts in ${RUN_MODE:-cli} mode${NC}"
    echo -e "      ${DIM}…or pick ${WHITE}NekoSuneAI${NC}${DIM} from your application menu.${NC}"
    echo -e "      ${DIM}Switch modes later: edit ${WHITE}.nekosuneai-run-mode${NC}${DIM} (gui / cli).${NC}"
    echo ""
...
main() {
    ...
    # Run mode (GUI vs CLI) — drives deps, system libs, and the launcher.
    choose_run_mode
    ...
    # Step 7: Launcher + desktop shortcut (both modes)
    create_launchers
    ...
    if ask_yes_no "Launch NekoSuneAI now?"; then
        echo ""
        info "Starting NekoSuneAI in ${RUN_MODE:-cli} mode..."
        local venv_python="$INSTALL_DIR/.venv/bin/python"
        # GUI mode needs a display; fall back to CLI if somehow headless.
        if [[ "$RUN_MODE" == "gui" ]] && ! is_headless; then
            (cd "$INSTALL_DIR" && "$venv_python" app.py --gui)
        else
            (cd "$INSTALL_DIR" && "$venv_python" app.py)
        fi
