...
.DESCRIPTION
    ...
    What the full wizard does:
        ...
        4. Clones or downloads NekoSuneAI
...
$REPO_BRANCH  = "main"
$INSTALL_DIR  = "$env:USERPROFILE\NekoSuneAI"
...
function Show-Banner {
    ...
    Write-Host "         " -NoNewline; Write-Host "o--" -ForegroundColor DarkMagenta -NoNewline; Write-Host "|" -ForegroundColor Magenta -NoNewline; Write-Host "  N e k o S u n e  " -ForegroundColor White -NoNewline; Write-Host "|" -ForegroundColor Magenta -NoNewline; Write-Host "--o" -ForegroundColor DarkMagenta
...
function Choose-LLMProvider {
    ...
    switch ($choice) {
        ...
        3 {
            ...
            Write-Info "Make sure LM Studio's local server is running before you start NekoSuneAI."
...
function Get-NekoSuneAI {
    ...
    if (Test-Path "$INSTALL_DIR\setup.py") {
        Write-Ok "NekoSuneAI already exists at $INSTALL_DIR"
...
function Run-Setup {
    ...
    Write-Step "5/7" "Running NekoSuneAI setup..."
...
function Choose-RunMode {
    ...
    if ($choice -eq 1) {
        ...
        Write-Ok "Run mode: CLI  (base + voice)"
    } else {
        $script:RunMode = "gui"
        $script:InstallProfile = "full"
        Write-Ok "Run mode: GUI  (base + voice + desktop GUI)"
    }
}
...
"@
    ...
    try {
        $launcherShortcutPath = "$INSTALL_DIR\NekoSuneAI.lnk"
        ...
        $launcherShortcut.Description = "NekoSuneAI - AI Companion Studio ($mode)"
    ...
    try {
        ...
        $shortcutPath = "$desktopPath\NekoSuneAI.lnk"
        ...
        $shortcut.Description = "NekoSuneAI - AI Companion Studio ($mode)"
