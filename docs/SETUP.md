...
Everything you need to get NekoSuneAI running on Windows or Linux.
...
> ⚠️ Make sure your NVIDIA drivers are up to date. NekoSuneAI auto-detects CUDA availability at startup.
...
With `AUTO_UPDATE_CHECK=true` and `AUTO_UPDATE_INSTALL=true` in `.env`, NekoSuneAI checks GitHub on startup and self-updates if a new version is available.
