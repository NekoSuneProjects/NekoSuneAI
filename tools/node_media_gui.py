...
class MediaControls:
    ...
    def _build_media_page(self):
        ...
        for row, (label, variable, name) in enumerate((("Microphone / loopback", self.input_device_var, "audio_input_combo"), ("Playback output", self.output_device_var, "audio_output_combo"))):
            ...
            combo = ttk.Combobox(audio, textvariable=variable, state="readonly", width=24)
        ...
        ttk.Button(actions, text="Refresh devices", command=self._refresh_audio_devices).pack(side="left")
        ttk.Button(actions, text="Record once", command=lambda: self._run_node_tool(lambda agent: agent.media.listen())).pack(side="left", padx=6)
        ttk.Button(actions, text="Stop audio", command=self._stop_audio).pack(side="right")
        ttk.Checkbutton(audio, text="Continuous listening", variable=self.media_settings["audio_listen_enabled"]).grid(row=3, column=0, columnspan=2, sticky="w", padx=22)
        ttk.Checkbutton(audio, text="Play game narration on Windows", variable=self.media_settings["windows_tts_enabled"]).grid(row=4, column=0, columnspan=2, sticky="w", padx=22)
        ...
        ttk.Button(audio, text="Speak through Pi", command=self._test_tts).grid(row=7, column=1, sticky="e", padx=22, pady=8)
        vision = self._card(page, "Gameplay capture")
        ttk.Checkbutton(vision, text="Send gameplay frames to Pi", variable=self.media_settings["game_vision_enabled"]).pack(anchor="w", padx=22, pady=8)
        ...
        ttk.Button(buttons, text="Analyse in 3 seconds", command=self._analyse_game).pack(side="left")
        ttk.Button(buttons, text="Select Tesseract", command=self._select_tesseract).pack(side="left", padx=6)
        ttk.Button(vision, text="Apply media settings", command=self._apply_media_settings).pack(anchor="e", padx=22, pady=8)
    ...
    def _build_vrchat_page(self):
        ...
        ttk.Checkbutton(osc, text="Enable local OSC on node start", variable=self.media_settings["vrchat_osc_enabled"]).grid(row=0, column=0, columnspan=2, sticky="w", padx=22, pady=8)
        ...
        ttk.Button(actions, text="Arm OSC", command=self._arm_osc).pack(side="left", padx=6)
        ...
        for column, (label, name, value) in enumerate((("Forward", "Vertical", 1), ("Back", "Vertical", -1), ("Left", "Horizontal", -1), ("Right", "Horizontal", 1), ("Jump", "Jump", 1))):
            ...
            ttk.Button(motion, text=label, command=lambda n=name, v=value: self._run_node_tool(lambda agent: self._osc(agent).pulse(n, v))).grid(row=0, column=column, sticky="ew", padx=2)
        self._field(osc, "Chatbox text", self.chatbox_var, 5)
        ttk.Button(osc, text="Send chatbox", command=self._send_chatbox).grid(row=6, column=1, sticky="e", padx=22, pady=8)
        self._field(osc, "Avatar parameter", self.avatar_parameter_var, 7)
        self._field(osc, "Value (JSON)", self.avatar_value_var, 8)
        ttk.Button(osc, text="Set parameter", command=self._set_avatar_parameter).grid(row=9, column=1, sticky="e", padx=22, pady=8)
